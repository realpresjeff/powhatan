export function createRaptorModel(options: {
    ...
    const {
        bodyColor = 0x6f7a6a,   // muted green/gray
        bellyColor = 0x9aa78f,
        eyeColor = 0x111111,
        clawColor = 0x1a1a1a,
        scale = 1,
    ...
    const belly = new THREE.Mesh(
        new THREE.BoxGeometry(1.6, 0.30, 0.55),
        bellyMat
    );
    ...
    const head = new THREE.Mesh(
        new THREE.BoxGeometry(0.75, 0.35, 0.35),
        bodyMat
    );
    ...
    function makeLeg(side: 1 | -1) {
        ...
        const foot = new THREE.Mesh(
            new THREE.BoxGeometry(0.45, 0.20, 0.30),
            bodyMat
        );
    ...
    for (let i = 0; i < tailCount; i++) {
        ...
        const seg = new THREE.Mesh(
            new THREE.BoxGeometry(
                THREE.MathUtils.lerp(0.8, 0.25, t),
                THREE.MathUtils.lerp(0.35, 0.18, t),
                THREE.MathUtils.lerp(0.35, 0.18, t)
            ),
            bodyMat
        );
