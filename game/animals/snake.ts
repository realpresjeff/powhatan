export function createSnakeModel(options: {
    ...
    const {
        bodyColor = 0x4f7a3d,   // green snake
        bellyColor = 0x9bbf7a,
        eyeColor = 0x111111,
    ...
    const head = new THREE.Mesh(
        new THREE.BoxGeometry(0.50, 0.22, 0.28),
        bodyMat
    );
    ...
    const snout = new THREE.Mesh(
        new THREE.BoxGeometry(0.25, 0.16, 0.22),
        bodyMat
    );
    ...
    for (let i = 0; i < segmentCount; i++) {
        ...
        const segment = new THREE.Mesh(
            new THREE.BoxGeometry(segmentLength, height, width),
            bodyMat
        );
        ...
        const belly = new THREE.Mesh(
            new THREE.BoxGeometry(segmentLength * 0.9, height * 0.4, width * 0.9),
            bellyMat
        );
    ...
    tailTip.position.set(
        -segmentCount * segmentLength - 0.10,
        0.18,
        0
    );
    snake.add(tailTip);
