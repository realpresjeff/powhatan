export function handBag() {
    if (!this.leftArm || !this.leftArm.forearm)
        return;
    if (!this.equipment)
        this.equipment = {};
    ...
    // Bag body
    const body = new THREE.Mesh(new THREE.BoxGeometry(1.2, 1.4, 0.8), this.materialDarkest);
    body.position.set(0.6, -0.2, 0.3);
    bag.add(body);
    // Handle
    const handle = new THREE.Mesh(new THREE.TorusGeometry(0.4, 0.1, 8, 16, Math.PI), this.materialDark);
