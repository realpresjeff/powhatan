...
function spawnFireCharge(caster) {
    const fireball = new THREE.Mesh(new THREE.SphereGeometry(0.4, 12, 12), new THREE.MeshStandardMaterial({
        emissive: 0xff4500,
        color: 0xaa2200
    }));
...
function spawnFireExplosion(target) {
    const explosion = new THREE.Mesh(new THREE.SphereGeometry(1.5, 16, 16), new THREE.MeshStandardMaterial({
        emissive: 0xff3300,
        transparent: true,
        opacity: 0.8
    }));
...
function spawnIceWave(caster, target) {
    const ice = new THREE.Mesh(new THREE.ConeGeometry(1, 3, 8), new THREE.MeshStandardMaterial({
        color: 0x99ccff,
        emissive: 0x3366ff
    }));
    const model = target.model || target.base;
...
function freezeTarget(target) {
    ...
    setTimeout(() => {
        ...
        model.traverse && model.traverse(mesh => {
            if (mesh.material)
                mesh.material.color.set(meshOriginalColor);
        });
    }, 5000);
}
function spawnLightning(caster, target) {
    const bolt = new THREE.Line(new THREE.BufferGeometry().setFromPoints([
...
function animateProjectile(projectile, caster) {
    var _a, _b;
    const target = ((_a = caster.followTarget) === null || _a === void 0 ? void 0 : _a.model) || ((_b = caster.currentTarget) === null || _b === void 0 ? void 0 : _b.model);
    if (!target)
        return;
