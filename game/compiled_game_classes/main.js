...
socket.socket.on('player_updated', player => {
    ...
    remote.character.walk(Object.assign(Object.assign({}, player.position), { targetRotation: player.rotation }));
});
...
socket.socket.on("monsters_snapshot", monsters => {
    monsters.forEach(monster => {
        const model = engine.spawn_monster(monster.position, monster.name, monster.data, monster.stationary);
        engine.monsters.set(monster.data.monsterId, Object.assign(Object.assign({}, monster), { model }));
    });
});
socket.socket.on("player_joined", player => {
    if (player.id === socket.socket.id)
        return;
    if (engine.players.has(player.id))
        return;
    engine.spawnPlayer(player.id, player.position, player.username);
});
...
socket.socket.on("item_picked_up", ({ id, pickedUpBy }) => {
    ...
    if (pickedUpBy === socket.socket.id) {
        engine.character.inventory.add_to_inventory(Object.assign({}, entry.data));
    }
});
...
socket.socket.on("combat_event", data => {
    ...
    (_a = target.character.healthBar) === null || _a === void 0 ? void 0 : _a.update();
    // 3️⃣ Handle death
    if (data.killed) {
...
socket.socket.on("trade_update", ({ offers }) => {
    ...
    renderTradeItems(offers[socket.socket.id], offers[tradePartnerId]);
});
function renderTradeItems(yours = [], theirs = []) {
    ...
    yourDiv.innerHTML = "";
    theirDiv.innerHTML = "";
    yours === null || yours === void 0 ? void 0 : yours.forEach(item => addItemIcon(yourDiv, item));
    theirs === null || theirs === void 0 ? void 0 : theirs.forEach(item => addItemIcon(theirDiv, item));
...
socket.socket.on("monster:spawn", (_a) => {
    var { monsterId, position, name, data, stationary } = _a, rest = __rest(_a, ["monsterId", "position", "name", "data", "stationary"]);
    const monster = engine.spawn_monster(position, name, data, stationary);
    engine.monsters.set(monsterId, Object.assign({ position, name, data, stationary, model: monster }, rest));
});
