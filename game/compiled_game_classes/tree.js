...
export class Tree {
    constructor(type, position, hasFruit, scene) {
        ...
        // Create trunk
        this.trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.8, 5, 8), this.trunkMaterial);
    ...
    createLeaves() {
        let leaves;
        if (this.type === "pine") {
            leaves = new THREE.Mesh(new THREE.ConeGeometry(3, 6, 8), this.leavesMaterial);
        }
        else {
            leaves = new THREE.Mesh(new THREE.SphereGeometry(3, 8, 8), this.leavesMaterial);
        }
    ...
    createFruits() {
        ...
        for (let i = 0; i < fruitCount; i++) {
            ...
            const fruit = new THREE.Mesh(new THREE.SphereGeometry(0.3, 8, 8), new THREE.MeshStandardMaterial({ color: "#FFA500" }) // Orange color for fruits
            );
            fruit.position.set(this.position.x + randomizedX - weight, 0, this.position.z + randomizedZ - weight);
            fruit.userData = { isFruit: true, tree: this.treeParent, name: "persimmon", pickupable: true, edible: true };
    ...
    cutTree(character, notification_bus) {
        ...
        let cuttingInterval = setInterval(() => {
            ...
            if (character.skills.agility.total_points_available < cost || character.inventory.inventory.length >= character.inventory.inventoryLimit) {
                ...
                character.startRegeneration('agility'); // Start the agility regeneration process
                character.startRegeneration('strength');
