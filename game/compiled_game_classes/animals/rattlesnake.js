export function createCoiledRattlesnakeModel(data, options = {}) {
    const { bodyColor = 0x8b7a4a, // sandy brown
    bellyColor = 0xb9a777, eyeColor = 0x111111, rattleColor = 0xd6c7a1, tongueColor = 0xb02020, scale = 1, } = options;
    ...
    for (let i = 0; i < segmentCount; i++) {
        ...
        const y = t * height;
        const segment = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.18, segRadius), bodyMat);
        segment.position.set(Math.cos(angle) * radius, 0.20 + y, Math.sin(angle) * radius);
        ...
        // Belly
        const belly = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.07, segRadius * 0.9), bellyMat);
    ...
    // ------------------------------------------------------------
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.50, 0.22, 0.30), bodyMat);
    head.position.set(0.0, 0.55, 0.0);
    head.rotation.y = 0; // +X
    snake.add(head);
    // Snout
    const snout = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.16, 0.22), bodyMat);
    ...
    head.add(eyeL, eyeR);
    // Tongue
    const tongue = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.03, 0.02), tongueMat);
    ...
    for (let i = 0; i < rattleCount; i++) {
        const r = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.10, 0.16, 8), rattleMat);
        r.rotation.z = Math.PI / 2;
        r.position.set(Math.cos(Math.PI * 2 * coilTurns) * radius - i * 0.14, 0.22, Math.sin(Math.PI * 2 * coilTurns) * radius);
        snake.add(r);
    }
    // ------------------------------------------------------------
    // Final setup
