export function createHawkModel(options = {}) {
    const { bodyColor = 0x6b4a2d, // reddish brown hawk body
    wingColor = 0x5a3c26, // darker wings
    headColor = 0xcbbba4, // pale hawk head
    beakColor = 0xdcc25b, // yellow beak
    eyeColor = 0x111111, legColor = 0xd9a14b, tailColor = 0x8b4f29, // red tail
    scale = 1, } = options;
    ...
    // ============================================================
    const body = new THREE.Mesh(new THREE.SphereGeometry(0.50, 16, 16), bodyMat);
    ...
    // ============================================================
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.30, 16, 16), headMat);
    ...
    // Beak — sharp hook
    const beak = new THREE.Mesh(new THREE.ConeGeometry(0.10, 0.28, 12), beakMat);
    beak.rotation.z = -Math.PI / 2;
    beak.position.set(0.24, -0.02, 0);
    head.add(beak);
    // Tip hook
    const hook = new THREE.Mesh(new THREE.SphereGeometry(0.04, 6, 6), beakMat);
    ...
    function makeWing(mirror = 1) {
        ...
        const shoulder = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.45, 0.34), wingMat);
        shoulder.position.set(0.02, 0.74, 0.28 * mirror);
        shoulder.rotation.z = -0.18 * mirror;
        wingGroup.add(shoulder);
        const midWing = new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.70, 0.10), wingMat);
    ...
    // ============================================================
    const tail = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.80, 0.20), tailMat);
    ...
    // LEGS & TALONS
    // ============================================================
    function makeLeg(zOffset) {
        const legGroup = new THREE.Group();
        const thigh = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.07, 0.24, 8), legMat);
        thigh.position.set(0, -0.12, 0);
        const lower = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.22, 8), legMat);
        lower.position.set(0, -0.20, 0);
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.30, 0.10, 0.20), legMat);
