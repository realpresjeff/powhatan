export function createCrocodileModel(options = {}) {
    const { bodyColor = 0x4f6a47, // crocodile green
    bellyColor = 0x9bb68a, // lighter underside
    eyeColor = 0x111111, toothColor = 0xffffff, scale = 1, } = options;
    ...
    // ============================================================
    const body = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.55, 0.75), bodyMat);
    body.position.set(0, 0.7, 0);
    croc.add(body);
    // Belly underside
    const belly = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.25, 0.70), bellyMat);
    belly.position.set(0, -0.20, 0);
    body.add(belly);
    // ============================================================
    // HEAD — rectangular crocodile head
    // ============================================================
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.95, 0.45, 0.45), bodyMat);
    head.position.set(1.35, 0.82, 0);
    croc.add(head);
    // =======================
    // LONG SNOUT
    // =======================
    const snout = new THREE.Mesh(new THREE.BoxGeometry(0.95, 0.35, 0.40), bodyMat);
    snout.position.set(0.70, -0.05, 0);
    head.add(snout);
    // =======================
    // TEETH — small cubes
    // =======================
    function makeTooth(x, y, z) {
        const tooth = new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.12, 0.10), toothMat);
    ...
    function makeLeg(x, z) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.40, 0.35, 0.40), bodyMat);
        upper.position.set(0, -0.20, 0);
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.15, 0.45), bodyMat);
        foot.position.set(0, -0.25, 0);
        // Three claws
        function claw(offsetZ) {
            const c = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.12, 0.12), toothMat);
    ...
    // ============================================================
    function makeScute(x, z) {
        const s = new THREE.Mesh(new THREE.BoxGeometry(0.30, 0.20, 0.30), bodyMat);
