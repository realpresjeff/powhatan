export function createRavenModel(options = {}) {
    const { bodyColor = 0x111111, beakColor = 0x0c0c0c, eyeColor = 0x222222, legColor = 0x1a1a1a, scale = 1, } = options;
    ...
    raven.add(body);
    const chest = new THREE.Mesh(new THREE.SphereGeometry(0.38, 16, 16), bodyMat);
    ...
    const beak = new THREE.Mesh(new THREE.ConeGeometry(0.09, 0.30, 12), beakMat);
    ...
    function makeWing(mirror = 1) {
        ...
        const shoulder = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.35, 0.30), bodyMat);
        shoulder.position.set(0.02, 0.72, 0.28 * mirror);
        shoulder.rotation.z = -0.10 * mirror;
        wingGroup.add(shoulder);
        const featherPanel = new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.80, 0.06), bodyMat);
    ...
    // TAIL
    // ============================================================
    const tail = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.60, 0.14), bodyMat);
    ...
    function makeLeg(xOffset, zOffset) {
        ...
        // Thigh
        const thigh = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.06, 0.22, 8), legMat);
        thigh.position.set(0, -0.11, 0);
        // Lower leg
        const lower = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.20, 8), legMat);
        lower.position.set(0, -0.18, 0);
        // Foot: pointing forward (+X)
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.09, 0.18), legMat);
