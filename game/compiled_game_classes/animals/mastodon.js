export function createMastodonModel(options = {}) {
    const { bodyColor = 0x6b5a4a, // dark brown mastodon fur
    furColor = 0x7a6a58, // lighter shag accents
    tuskColor = 0xf0eadf, // ivory
    eyeColor = 0x111111, scale = 1, } = options;
    ...
    // ------------------------------------------------------------
    const body = new THREE.Mesh(new THREE.BoxGeometry(3.2, 1.8, 1.6), bodyMat);
    body.position.set(0, 1.9, 0);
    mastodon.add(body);
    // Fur hump
    const hump = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.9, 1.4), furMat);
    ...
    mastodon.add(head);
    // Forehead dome
    const dome = new THREE.Mesh(new THREE.BoxGeometry(1.1, 0.7, 1.0), furMat);
    ...
    for (let i = 0; i < trunkCount; i++) {
        const seg = new THREE.Mesh(new THREE.CylinderGeometry(0.20 - i * 0.03, 0.22 - i * 0.03, 0.6, 10), bodyMat);
        seg.rotation.z = Math.PI / 2;
        seg.position.set(0.9 + i * 0.45, -0.4 - i * 0.1, 0);
        head.add(seg);
    ...
    function makeTusk(side) {
        const tusk = new THREE.Mesh(new THREE.CylinderGeometry(0.10, 0.14, 1.4, 12), tuskMat);
    ...
    function makeEar(side) {
        const ear = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.90, 0.90), furMat);
    ...
    function makeLeg(x, z) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.4, 0.6), bodyMat);
        upper.position.set(0, -0.7, 0);
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.75, 0.35, 0.75), bodyMat);
    ...
    // ------------------------------------------------------------
    const tail = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.35, 0.35), furMat);
