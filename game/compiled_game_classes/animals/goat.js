export function createGoatModel(options = {}) {
    const { bodyColor = 0xcabfaa, // light brown/tan goat
    bellyColor = 0xdfd7c9, headColor = 0xc3b59a, hornColor = 0x6d6a63, eyeColor = 0x111111, hoofColor = 0x3d3a38, scale = 1, } = options;
    ...
    // ============================================================
    const body = new THREE.Mesh(new THREE.SphereGeometry(0.65, 18, 18), bodyMat);
    body.position.set(0, 0.80, 0);
    goat.add(body);
    // Belly
    const belly = new THREE.Mesh(new THREE.SphereGeometry(0.55, 18, 18), bellyMat);
    ...
    // HEAD
    // ============================================================
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.40, 16, 16), headMat);
    head.position.set(0.75, 1.15, 0);
    goat.add(head);
    // Snout
    const snout = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.25, 0.30), headMat);
    snout.position.set(0.30, -0.05, 0);
    head.add(snout);
    // Nose
    const nose = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.12, 0.14), hoofMat);
    nose.position.set(0.20, -0.03, 0);
    snout.add(nose);
    // Beard
    const beard = new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.30, 0.12), headMat);
    ...
    function makeHorn(mirror = 1) {
        const horn = new THREE.Mesh(new THREE.CylinderGeometry(0.10, 0.12, 0.45, 8), hornMat);
        horn.rotation.z = Math.PI / 2;
        horn.rotation.y = 0.4 * mirror;
        horn.position.set(0.00, 0.22, 0.22 * mirror);
        const curve = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.10, 0.40, 8), hornMat);
    ...
    function makeFrontLeg(zOffset) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.45, 0.18), bodyMat);
        upper.position.set(0, -0.22, 0);
        const hoof = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.15, 0.20), hoofMat);
    ...
    function makeBackLeg(zOffset) {
        const leg = new THREE.Group();
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.45, 0.20), bodyMat);
        upper.position.set(0, -0.22, 0);
        upper.rotation.x = 0.25; // angled backwards
        const lower = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.40, 0.16), bodyMat);
