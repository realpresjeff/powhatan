export function createHorseModel(data, options = {}) {
    const { coatColor = 0x8c6239, // brown
    maneColor = 0x3a2c20, // dark mane
    eyeColor = 0x111111, hoofColor = 0x2a2624, scale = 1, } = options;
    ...
    // ============================================================
    const body = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.85, 0.55), coatMat);
    body.userData = data;
    body.position.set(0, 1.1, 0);
    horse.add(body);
    // Chest (slightly deeper)
    const chest = new THREE.Mesh(new THREE.BoxGeometry(0.75, 0.80, 0.60), coatMat);
    chest.position.set(0.60, 1.08, 0);
    horse.add(chest);
    // ============================================================
    // NECK — angled, long, elegant
    // ============================================================
    const neck = new THREE.Mesh(new THREE.BoxGeometry(0.45, 1.00, 0.45), coatMat);
    ...
    // ============================================================
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.75, 0.45, 0.40), coatMat);
    head.position.set(1.45, 1.95, 0);
    head.rotation.z = -0.10;
    horse.add(head);
    // Muzzle
    const muzzle = new THREE.Mesh(new THREE.BoxGeometry(0.40, 0.28, 0.32), coatMat);
    muzzle.position.set(0.40, -0.02, 0);
    head.add(muzzle);
    // Nostrils (small cubes)
    const nostrilL = new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.10, 0.10), hoofMat);
    ...
    function makeEar(mirror = 1) {
        const ear = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.35, 0.12), coatMat);
    ...
    // horse.add(mane);
    // Forelock (front mane)
    const forelock = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.32, 0.25), maneMat);
    forelock.position.set(1.15, 1.95, 0);
    horse.add(forelock);
    // ============================================================
    // LEGS — long, straight, realistic
    // ============================================================
    function makeFrontLeg(zOffset) {
        const leg = new THREE.Group();
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.70, 0.22), coatMat);
        upper.position.set(0, -0.35, 0);
        const lower = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.70, 0.18), coatMat);
        lower.position.set(0, -0.55, 0);
        const hoof = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.20, 0.24), hoofMat);
    ...
    function makeBackLeg(zOffset) {
        const leg = new THREE.Group();
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.75, 0.25), coatMat);
        upper.position.set(0, -0.35, 0);
        upper.rotation.x = 0.22; // realistic back-leg angle
        const lower = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.70, 0.20), coatMat);
        lower.position.set(0, -0.55, -0.05);
        const hoof = new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.22, 0.26), hoofMat);
    ...
    // ============================================================
    const tail = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.90, 0.25), maneMat);
