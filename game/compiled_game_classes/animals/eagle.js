export function createEagleModel(options = {}) {
    const { bodyColor = 0x4a3728, // brown body
    ...
    eyeColor = 0x111111, tailColor = 0xffffff, // white tail feathers
    scale = 1, } = options;
    ...
    // Beak
    const beak = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.30, 12), beakMat);
    beak.rotation.z = -Math.PI / 2;
    beak.position.set(0.24, -0.03, 0);
    head.add(beak);
    // Hook tip
    const hook = new THREE.Mesh(new THREE.SphereGeometry(0.05, 8, 8), beakMat);
    ...
    function makeWing(mirror = 1) {
        const wing = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.9, 0.55), wingMat);
    ...
    // TAIL FEATHERS
    // ============================================================
    const tail = new THREE.Mesh(new THREE.BoxGeometry(0.30, 0.50, 0.15), tailMat);
    ...
    function makeLeg(xOffset) {
        ...
        const thigh = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.12, 0.25, 8), talonMat);
        thigh.position.set(0, -0.12, 0);
        const lower = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.07, 0.28, 8), talonMat);
        lower.position.set(0, -0.22, 0);
        const talon = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.10, 0.30), talonMat);
