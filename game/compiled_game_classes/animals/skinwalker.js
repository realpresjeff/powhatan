export function createSkinwalkerDeerModel(options = {}) {
    const { skinColor = 0x3a2e27, // dark creature skin
    boneColor = 0xded2c2, // deer skull color
    antlerColor = 0xbba78c, eyeColor = 0x111111, clawColor = 0x1a1a1a, scale = 1, } = options;
    ...
    // ============================================================
    const torso = new THREE.Mesh(new THREE.BoxGeometry(0.8, 1.6, 0.5), skinMat);
    torso.position.set(0, 1.6, 0);
    creature.add(torso);
    // Rib accents (creepy)
    function rib(yOffset, width) {
        const r = new THREE.Mesh(new THREE.BoxGeometry(width, 0.15, 0.45), boneMat);
    ...
    // ============================================================
    const skull = new THREE.Mesh(new THREE.BoxGeometry(0.65, 0.75, 0.45), boneMat);
    skull.position.set(0, 2.55, 0);
    creature.add(skull);
    // Snout (elongated)
    const snout = new THREE.Mesh(new THREE.BoxGeometry(0.40, 0.35, 0.35), boneMat);
    ...
    function makeAntler(mirror = 1) {
        const base = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.70, 0.15), antlerMat);
        base.position.set(-0.20, 0.40, 0.30 * mirror);
        base.rotation.z = mirror * 0.25;
        const branch1 = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.45, 0.12), antlerMat);
        branch1.position.set(0, 0.30, 0.20 * mirror);
        branch1.rotation.z = mirror * 0.35;
        const branch2 = new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.35, 0.10), antlerMat);
    ...
    function makeArm(mirror = 1) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.95, 0.25), skinMat);
        upper.position.set(0, -0.50, 0);
        const lower = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.90, 0.20), skinMat);
        lower.position.set(0, -0.75, 0);
        upper.add(lower);
        const hand = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.18, 0.35), skinMat);
        hand.position.set(0, -0.45, 0);
        lower.add(hand);
        // Claws
        function claw(offsetZ) {
            const c = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.28, 0.12), clawMat);
    ...
    function makeLeg(mirror = 1) {
        ...
        const thigh = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.90, 0.35), skinMat);
        thigh.position.set(0, -0.45, 0);
        thigh.rotation.x = 0.25;
        const shin = new THREE.Mesh(new THREE.BoxGeometry(0.30, 0.90, 0.30), skinMat);
        shin.position.set(0, -0.75, -0.10);
        thigh.add(shin);
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.22, 0.45), skinMat);
        foot.position.set(0, -0.45, 0.05);
        shin.add(foot);
        // Claws
        function toe(zOff) {
            const t = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.20, 0.14), clawMat);
