export function createFrogModel(options = {}) {
    const { bodyColor = 0x4b8f3a, bellyColor = 0xa8d08d, eyeColor = 0x111111, toeColor = 0x333333, scale = 1, } = options;
    ...
    // ============================================================
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.55, 18, 18), bodyMat);
    head.position.set(0.25, 1.05, 0);
    frog.add(head);
    // Mouth line
    const mouth = new THREE.Mesh(new THREE.BoxGeometry(0.85, 0.08, 0.85), toeMat);
    ...
    function makeFrontLeg(zOffset) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.30, 0.25), bodyMat);
        upper.position.set(0, -0.20, 0);
        const lower = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.25, 0.22), bodyMat);
        lower.position.set(0, -0.25, 0.02);
        const toes = new THREE.Mesh(new THREE.BoxGeometry(0.40, 0.10, 0.40), toeMat);
    ...
    function makeBackLeg(zOffset) {
        const leg = new THREE.Group();
        const thigh = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.50, 0.45), bodyMat);
        thigh.position.set(0, -0.30, 0);
        thigh.rotation.x = 0.35;
        const calf = new THREE.Mesh(new THREE.BoxGeometry(0.40, 0.45, 0.40), bodyMat);
        calf.position.set(0, -0.42, -0.05);
        const toes = new THREE.Mesh(new THREE.BoxGeometry(0.70, 0.12, 0.70), toeMat);
