export function createDutchMarine1575Model(options = {}) {
    const { coatColor = 0x3f4f5a, // blue-gray coat
    pantsColor = 0x5a5a4a, // canvas trousers
    hatColor = 0x2a2a2a, // dark felt cap
    leatherColor = 0x5a3b1e, skinColor = 0xd6c4a4, eyeColor = 0x111111, woodColor = 0x6b4a2d, metalColor = 0x666666, scale = 1, } = options;
    ...
    // ============================================================
    const torso = new THREE.Mesh(new THREE.BoxGeometry(0.65, 0.95, 0.45), coatMat);
    ...
    const cap = new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.18, 0.48), hatMat);
    ...
    const stock = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.14, 0.12), woodMat);
    gun.add(stock);
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 1.6, 10), metalMat);
    barrel.rotation.z = Math.PI / 2;
    barrel.position.set(0.8, 0, 0);
    gun.add(barrel);
    const lock = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.10, 0.10), metalMat);
    ...
    function makeLeg(side) {
        ...
        thigh.add(shin);
        const shoe = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.12, 0.45), leatherMat);
