export function createTurtleModel(options = {}) {
    const { shellColor = 0x4a5f39, // olive green shell
    ...
    // ============================================================
    const shellBase = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.45, 1.15), shellMat);
    shellBase.position.set(0, 0.75, 0);
    turtle.add(shellBase);
    // Plates on top (three segments)
    function makePlate(x, z) {
        const plate = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.20, 0.45), plateMat);
    ...
    // ============================================================
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.40, 0.40), bodyMat);
    head.position.set(1.0, 0.82, 0);
    turtle.add(head);
    // Beak (turtle mouth)
    const beak = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.15, 0.30), bodyMat);
    ...
    function makeLeg(x, z) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.35, 0.35), bodyMat);
        upper.position.set(0, -0.18, 0);
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.38, 0.18, 0.38), bodyMat);
        foot.position.set(0, -0.22, 0);
        // Simple claws: three box bumps
        function makeClaw(offsetZ) {
            const claw = new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.10, 0.10), plateMat);
    ...
    // TAIL — small box tail
    // ============================================================
    const tail = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.22, 0.22), bodyMat);
