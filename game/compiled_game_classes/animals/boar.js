export function createBoarModel(options = {}) {
    const { bodyColor = 0x5a3b2e, // dark brown
    snoutColor = 0x8a6a55, // lighter snout
    tuskColor = 0xe6e6e6, // white tusks
    hoofColor = 0x2d2a28, eyeColor = 0x111111, bristleColor = 0x3b2b20, scale = 1, } = options;
    ...
    // ============================================================
    const body = new THREE.Mesh(new THREE.SphereGeometry(0.70, 20, 20), bodyMat);
    body.position.set(0, 0.75, 0);
    boar.add(body);
    // Chest bulge
    const chest = new THREE.Mesh(new THREE.SphereGeometry(0.55, 18, 18), bodyMat);
    chest.position.set(0.20, 0.70, 0.10);
    boar.add(chest);
    // ============================================================
    // HEAD — boar wedge shape
    // ============================================================
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.45, 0.45), bodyMat);
    head.position.set(0.75, 1.00, 0);
    head.rotation.z = 0.1;
    boar.add(head);
    // Snout
    const snout = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.20, 0.45, 12), snoutMat);
    snout.rotation.z = -Math.PI / 2;
    snout.position.set(0.32, -0.02, 0);
    head.add(snout);
    // Snout tip noseplate
    const nose = new THREE.Mesh(new THREE.SphereGeometry(0.14, 8, 8), snoutMat);
    nose.position.set(0.22, 0, 0);
    snout.add(nose);
    // Tusks
    function makeTusk(mirror = 1) {
        const tusk = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.07, 0.25, 8), tuskMat);
    ...
    function makeEar(mirror = 1) {
        const ear = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.28, 0.12), bodyMat);
    ...
    // ============================================================
    const bristle = new THREE.Mesh(new THREE.BoxGeometry(0.10, 0.55, 0.25), bristleMat);
    ...
    // ============================================================
    function makeLeg(xOffset, zOffset) {
        const legGroup = new THREE.Group();
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.45, 0.22), bodyMat);
        upper.position.set(0, -0.22, 0);
        const hoof = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.20, 0.22), hoofMat);
    ...
    // TAIL — little curl
    // ============================================================
    const tail = new THREE.Mesh(new THREE.TorusGeometry(0.10, 0.04, 8, 16), bodyMat);
