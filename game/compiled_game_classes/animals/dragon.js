export function createDragonModel(monsterData, options = {}) {
    const { bodyColor = 0x4a2f2f, // dark red/brown dragon scales
    bellyColor = 0x8b5a4a, // lighter underside
    wingColor = 0x3b1f1f, hornColor = 0xf0e6d2, eyeColor = 0x111111, scale = 1, } = options;
    ...
    // ============================================================
    const body = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.70, 0.75), bodyMat);
    monsterData.name = "Jersey Devil";
    body.userData = monsterData;
    body.position.set(0, 1.1, 0);
    dragon.add(body);
    // Belly plates
    const belly = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.30, 0.70), bellyMat);
    belly.position.set(0, -0.25, 0);
    body.add(belly);
    // ============================================================
    // NECK — long box neck
    // ============================================================
    const neck = new THREE.Mesh(new THREE.BoxGeometry(0.40, 1.10, 0.45), bodyMat);
    ...
    // ============================================================
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.85, 0.45, 0.45), bodyMat);
    head.position.set(1.55, 1.95, 0);
    head.rotation.z = -0.10;
    dragon.add(head);
    // SNOUT
    const snout = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.32, 0.40), bodyMat);
    snout.position.set(0.45, -0.02, 0);
    head.add(snout);
    // TEETH
    function makeFang(offsetZ) {
        const fang = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.20, 0.12), hornMat);
    ...
    // BACK SPIKES
    function makeSpike(x, y, z) {
        const spike = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.40, 0.22), hornMat);
    ...
    function makeLeg(x, z) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.30, 0.50, 0.30), bodyMat);
        upper.position.set(0, -0.25, 0);
        const lower = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.45, 0.25), bodyMat);
        lower.position.set(0, -0.40, 0);
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.20, 0.35), bellyMat);
    ...
    function makeWing(mirror = 1) {
        ...
        const bone = new THREE.Mesh(new THREE.BoxGeometry(0.80, 0.15, 0.15), wingMat);
        const membrane = new THREE.Mesh(new THREE.BoxGeometry(0.80, 0.50, 0.02), wingMat);
    ...
    // TAIL — long taper
    // ============================================================
    function tailSegment(size, x) {
        const seg = new THREE.Mesh(new THREE.BoxGeometry(size, 0.30, 0.30), bodyMat);
