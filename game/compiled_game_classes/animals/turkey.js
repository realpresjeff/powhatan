export function createTurkeyModel(options = {}) {
    const { bodyColor = 0x4b2e1a, // dark brown
    ...
    // ============================================================
    const body = new THREE.Mesh(new THREE.SphereGeometry(0.7, 24, 24), bodyMat);
    body.position.set(0, 0.75, 0);
    turkey.add(body);
    // Chest puff
    const chest = new THREE.Mesh(new THREE.SphereGeometry(0.55, 24, 24), bodyMat);
    ...
    // Beak
    const beak = new THREE.Mesh(new THREE.ConeGeometry(0.10, 0.25, 8), beakMat);
    beak.rotation.z = -Math.PI / 2;
    beak.position.set(0.21, -0.02, 0);
    head.add(beak);
    // Wattle (red dangly thing)
    const wattle = new THREE.Mesh(new THREE.SphereGeometry(0.07, 8, 8), wattleMat);
    ...
    function makeWing(mirror = 1) {
        const wing = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.6, 0.45), featherMat);
    ...
    for (let i = -4; i <= 4; i++) {
        const feather = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.90, 0.05), i === -4 || i === 4 ? tailTipMat : featherMat);
    ...
    function makeLeg(xOffset) {
        ...
        const upper = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.10, 0.35, 8), legMat);
        upper.position.set(0, -0.18, 0);
        const lower = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.07, 0.25, 8), legMat);
        lower.position.set(0, -0.30, 0);
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.10, 0.30), legMat);
