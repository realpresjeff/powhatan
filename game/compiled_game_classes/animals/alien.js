export function createGrayAlienModel(options = {}) {
    const { skinColor = 0x9ea2a3, // light gray
    ...
    // TORSO – facing +X
    // ------------------------------------------------------------
    const torso = new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.2, 0.4), skinMat);
    torso.position.set(0, 1.2, 0); // center torso at about 1.2 high
    alien.add(torso);
    // Slight abdomen
    const abdomen = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.6, 0.32), skinMat);
    ...
    function makeArm(side) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.7, 0.16), skinMat);
        upper.position.set(0, -0.35, 0);
        const lower = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.65, 0.14), skinMat);
        lower.position.set(0, -0.55, 0);
        upper.add(lower);
        const hand = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.14, 0.2), skinMat);
        hand.position.set(0, -0.38, 0);
        lower.add(hand);
        // simple three fingers
        function finger(zOff) {
            const f = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.22, 0.06), skinMat);
    ...
    function makeLeg(side) {
        ...
        const thigh = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.85, 0.2), skinMat);
        thigh.position.set(0, -0.42, 0);
        const shin = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.8, 0.16), skinMat);
        shin.position.set(0, -0.7, 0);
        thigh.add(shin);
        const foot = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.18, 0.35), skinMat);
