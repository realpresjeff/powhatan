export function createAnkylosaurusModel(options = {}) {
    const { bodyColor = 0x6f7f6a, // olive gray
    armorColor = 0x8a6b4a, // darker armor
    bellyColor = 0x9cab8a, eyeColor = 0x111111, scale = 1, } = options;
    ...
    anky.add(body);
    const belly = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.35, 1.5), bellyMat);
    belly.position.set(0, -0.30, 0);
    body.add(belly);
    // ------------------------------------------------------------
    // HEAD — small, armored
    // ------------------------------------------------------------
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.75, 0.45, 0.55), bodyMat);
    head.position.set(1.9, 1.15, 0);
    anky.add(head);
    // Snout
    const snout = new THREE.Mesh(new THREE.BoxGeometry(0.30, 0.25, 0.35), bodyMat);
    ...
    for (let i = 0; i < armorCount; i++) {
        const plate = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.30, 0.60), armorMat);
        plate.position.set(-0.9 + i * 0.45, 1.70, i % 2 === 0 ? 0.55 : -0.55);
        anky.add(plate);
    }
    // ------------------------------------------------------------
    // LEGS — short, thick
    ...
    function makeLeg(x, z) {
        ...
        const upper = new THREE.Mesh(new THREE.BoxGeometry(0.45, 0.8, 0.45), bodyMat);
    ...
    for (let i = 0; i < tailCount; i++) {
        const seg = new THREE.Mesh(new THREE.BoxGeometry(0.7 - i * 0.15, 0.35, 0.35), bodyMat);
        seg.position.set(-1.8 - i * 0.45, 1.1, 0);
        anky.add(seg);
        tailSegments.push(seg);
    }
    // Tail club
    const club = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.45, 0.6), armorMat);
