...
import { Monster, Summon } from './monster.js';
...
export class Character {
    ...
    addBody(shirt = false, female = false) {
        ...
        const body = new THREE.Mesh(
            bodyGeo,
            shirt ? this.materialDark : this.skinMaterial
        );
    ...
    walk(positionData = null) {
        ...
        } else this.currentRotation += (this.targetRotation - this.currentRotation) * 0.5;
    ...
    attack(target) {
        ...
        if (this.activeSpell) return this.attackWithMagic(target, this.activeSpell);
        ...
        if (leftHandItem && leftHandItem.name.includes("rifle") || leftHandItem.name.includes("gun") || leftHandItem.name.includes("pistol")
            ||
            rightHandItem && rightHandItem.name && rightHandItem.name.includes("rifle") || rightHandItem?.name?.includes("gun") || rightHandItem?.name?.includes("pistol")) {
        ...
        if (this.isThisPlayer) {
            if (target.monster) {
                return socket.sendMessage("combat_event", {
                    ...
                    attackType: "melee",
                    weapon: this.equippedItems.left_hand?.name || null,
                    damage,
        ...
        function createArrowMesh() {
            ...
            for (let i = 0; i < 3; i++) {
                const feather = new THREE.Mesh(
                    new THREE.BoxGeometry(0.15, 0.05, 0.35),
                    featherMat
                );
    ...
    attackWithArcher(target, arrow) {
        ...
        const projectileTarget = target.model ?
            target.model.position?.clone()?.add(new THREE.Vector3(0, 2, 0))
            : target.base.position?.clone()?.add(new THREE.Vector3(0, 2, 0));
    ...
    smelt() {
        ...
        this.inventory.inventory.forEach((item: Item, index) => {
    ...
    equipItem(item: Item) {
        ...
        if (slot && Array.isArray(slot)) {
            ...
            if (!wornSlot1 || !wornSlot2) return console.warn(
                `EquipType is ${item.equipType}, but slot 1 is: ${wornSlot1} and slot 2 is: ${wornSlot2}`
            );
    ...
    unequipItem(item: Item) {
        ...
        if (this.equippedItems[slotName]) {
            ...
            this.inventory.addToInventory(
                this.equippedItems[slotName].name
    ...
    follow(targetCharacter, distance = 2.5) {
        if (!targetCharacter || !targetCharacter.base) return;
    ...
    createHair(style, color) {
        ...
        if (!isBald) {
            ...
            switch (hairStyle) {
                case "short":
                    hair = new THREE.Mesh(
                        new THREE.BoxGeometry(1.7, 0.6, 1.4),
                        hairMaterial
                    );
    ...
    summonMonster(item) {
        ...
        // Override stats
        summon.stats = { ...item.sealedMonster.stats };
