export function createDuckModel(options: {
    ...
    const {
        bodyColor = 0x8fae6a,   // greenish/brown duck body
        wingColor = 0x7a915c,
        billColor = 0xe2a13a,   // orange bill
        eyeColor = 0x111111,
        footColor = 0xe2a13a,
        scale = 1,
    } = options;
    ...
    const bill = new THREE.Mesh(
        new THREE.BoxGeometry(0.35, 0.14, 0.28),
        billMat
    );
    ...
    // WINGS — tucked
    // ============================================================
    function makeWing(side: 1 | -1) {
        const wing = new THREE.Mesh(
            new THREE.BoxGeometry(0.55, 0.30, 0.10),
            wingMat
        );
    ...
    const tail = new THREE.Mesh(
        new THREE.BoxGeometry(0.25, 0.20, 0.20),
        bodyMat
    );
    ...
    function makeLeg(side: 1 | -1) {
        ...
        const foot = new THREE.Mesh(
            new THREE.BoxGeometry(0.30, 0.06, 0.22),
            footMat
        );
