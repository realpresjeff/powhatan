export function createStegosaurusModel(options: {
    ...
    const {
        bodyColor = 0x6f8b5a,   // greenish body
    ...
    const belly = new THREE.Mesh(
        new THREE.BoxGeometry(2.8, 0.4, 1.1),
        bellyMat
    );
    ...
    const snout = new THREE.Mesh(
        new THREE.BoxGeometry(0.30, 0.22, 0.28),
        bodyMat
    );
    ...
    for (let i = 0; i < plateCount; i++) {
        const plate = new THREE.Mesh(
            new THREE.BoxGeometry(0.20, 0.80, 0.60),
            plateMat
        );
        ...
        plate.position.set(
            -0.8 + i * 0.5,
            1.95,
            i % 2 === 0 ? 0.35 : -0.35 // alternating rows
        );
    ...
    for (let i = 0; i < tailCount; i++) {
        const seg = new THREE.Mesh(
            new THREE.BoxGeometry(
                THREE.MathUtils.lerp(0.8, 0.3, i / (tailCount - 1)),
                0.35,
                0.35
            ),
            bodyMat
