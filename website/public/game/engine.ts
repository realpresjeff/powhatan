...
export class Engine {
    ...
    character = new Character(this.scene, true);
    ...
    notification_bus = new Notifications(this.character.name, this.character.base);

    spellbook = new Spellbook(this.character);
    ...
    spawn_monster(position: { x: number, y: number, z: number }, name: string, data: any, stationary: boolean) {
    ...
    create_dropped_item(itemData) {
        ...
        if (itemData) {
            ...
            item.userData = { ...itemData.item, id: itemData.id };
            // items = { ...items, [item.uuid]: item }

            // Position the item exactly where the player is standing
            item.position.set(
                this.player.position.x,
                0, // slightly above ground so it doesn't clip
            ...
            this.character.inventory.removeFromInventory(itemData);
